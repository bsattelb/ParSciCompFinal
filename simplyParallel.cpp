...
int main(int argc, char* argv[]) {
  ...
  inFile->seekg (offset_vec[my_rank], inFile->beg);
	...
	MPI::COMM_WORLD.Gatherv( partOfTheText, count_vec[my_rank], offset_vec,
                           MPI::CHAR, allOfTheText, count_vec, MPI::CHAR, 0);

  if( my_rank == 0) {
    writeOut(fullText, outFile);
  }
